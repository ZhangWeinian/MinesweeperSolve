...
def run_auto_bot(rows: int = 16, cols: int = 30, total_mines: int = 99, model_path=None, meta_path=None):
    ...
    while not state.stop:
        ...
        if action in ("CLICK", "FLAG"):
            ...
            section_action.append(f"  基准存活率 : {C.CYAN}{baseline_surv:.2f}%{C.RESET} (全局盲狙黑区概率)")
            ...
            center = capture.get_cell_center(best_r, best_c)
            if center is None:
                print(f"{C.RED}❌ 坐标 ({best_r}, {best_c}) 未被正确初始化，跳过本回合...{C.RESET}")
                continue
            best_target_x, best_target_y = center
            ...
            if state.decision == "left":
                ...
                click(best_target_x, best_target_y)
                time.sleep(0.2)
            elif state.decision == "right":
                print(f"🤖 {C.RED}接收指令：自动代劳 [右键标雷]...{C.RESET}")
                right_click(best_target_x, best_target_y)
                time.sleep(0.2)
