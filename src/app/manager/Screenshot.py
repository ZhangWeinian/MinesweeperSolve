...
if sys.platform == "win32":
    ...
    def _get_physical_mouse_pos() -> tuple[int, int]:
    ...
    class ScreenCapture:
        ...
        def _find_true_center(self, sct_img: np.ndarray, raw_x: int, raw_y: int) -> tuple[int, int]:
        ...
        def get_cell_image(self, frame: np.ndarray, r: int, c: int) -> np.ndarray:
    ...
    class ScreenCapture:
        """非 Windows 环境存根类，仅用于通过 IDE 静态检查，不可实例化运行。"""

        def __init__(self, *args, **kwargs):
            """非 Windows 环境存根构造函数，参数无实际意义，仅用于签名匹配。"""
            pass

        def grab_frame(self) -> Any:
            """非 Windows 环境存根方法，返回 None，仅用于签名匹配。"""
            return None

        def _find_true_center(self, *args, **kwargs) -> tuple[int, int]:
            """非 Windows 环境存根方法，返回 (0, 0)，仅用于签名匹配。"""
            return (0, 0)

        def calibrate(self) -> None:
            """非 Windows 环境存根方法，无实际功能，仅用于签名匹配。"""
            pass

        def get_cell_center(self, *args, **kwargs) -> tuple[int, int] | None:
            """非 Windows 环境存根方法，返回 None，仅用于签名匹配。"""
            return None

        def get_cell_image(self, *args, **kwargs) -> Any:
            """非 Windows 环境存根方法，返回 None，仅用于签名匹配。"""
            return None
